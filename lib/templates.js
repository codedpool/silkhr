...
// Patch a subset of editable fields. Owner check baked in.
const EDITABLE_FIELDS = [
  'name', 'role', 'jobDescription', 'resumeHighlights',
  'interviewType', 'durationMinutes',
  'pressureMode', 'hinglishMode', 'deepFollowups',
];

export async function updateTemplate(templateId, ownerId, patch) {
  const safe = {};
  for (const k of EDITABLE_FIELDS) {
    if (patch[k] === undefined) continue;
    if (typeof patch[k] === 'string')  safe[k] = patch[k].trim();
    else if (typeof patch[k] === 'number') safe[k] = patch[k];
    else if (typeof patch[k] === 'boolean') safe[k] = patch[k];
  }
  if (Object.keys(safe).length === 0) return getTemplate(templateId);
  safe.updatedAt = Date.now();
  const col = await getCol();
  await col.updateOne({ _id: templateId, ownerId }, { $set: safe });
  return col.findOne({ _id: templateId });
}
